...
class Server:
    ...
    def predict_action(self, payload: Dict[str, Any]) -> JSONResponse:
        ...
        try:
            ...
            if not self.enable_rtc:
                ...
                ### a single synchronous client. Only this branch is reachable (assert above).
                with self._predict_lock:
                    raw_pred_actions = self.model.predict_action(
                        ...
                        states=torch.zeros_like(states.unsqueeze(0)), ### ZERO OUT ###
            ...
            pred_actions[..., -5] = 0.75 # need to hardcode 0.75 for torso height here? ### ZERO OUT ###
    ...
    ### CLAUDE ### -----------------------------------------------------------------
    ### Stateless real-time-chunking (RTC) endpoint for world-model closed-loop rollouts.
    ###
    ### This exposes `predict_action_with_training_rtc_flow` (the same call the real-robot
    ### RTC server `psi_serve_rtc-trainingtimertc_zeros.py` uses) WITHOUT the wall-clock
    ### control loop / threads. The CLIENT owns the RTC state machine (A_cur, the executed
    ### counter, the delay) and passes it in per request:
    ###   - condition["prev_actions"]     : (H, D) NORMALISED previous chunk, already shifted
    ###                                      by the executed count and zero-padded — i.e.
    ###                                      concat([A_cur[s:], zeros((s, D))]). Omit/empty on
    ###                                      the first call (a plain predict_action is used).
    ###   - condition["inference_delay"]  : d  (soft-mask overlap; the RTC flow freezes the
    ###                                      first d actions of the new chunk to prev_actions).
    ###   - condition["max_delay"]        : max_delay (must satisfy d < max_delay, d < H).
    ###
    ### Returns BOTH representations of the full (H, D) chunk so the client never needs the
    ### server's stats: "action_norm" (feed back as prev_actions next round) and
    ### "action_denorm" (absolute joint targets, for the world model). States are zeroed to
    ### match the _zeros deployment convention.
    def predict_action_rtc(self, payload: Dict[str, Any]) -> JSONResponse:
        try:
            request = RequestMessage.deserialize(payload)
            image_dict, instruction, state_dict = request.image, request.instruction, request.state
            cond = request.condition or {}
            prev_actions = cond.get("prev_actions", None)
            inference_delay = int(cond.get("inference_delay", 0))
            max_delay = int(cond.get("max_delay", 0))

            overwatch.info(f"[rtc] Instruction: {instruction}  d={inference_delay}  max_delay={max_delay}")
            ...
            states = torch.zeros_like(states.unsqueeze(0))  # (1, 1, Ds) ### ZERO OUT ###

            has_prev = prev_actions is not None and np.asarray(prev_actions).size > 0
            with self._predict_lock:
                if not has_prev:
                    # First plan of the episode: plain predict_action (no RTC constraint), like
                    # RealTimeChunkController._predict_action in the real-robot server.
                    ...
                    pa = torch.from_numpy(np.asarray(prev_actions, dtype=np.float32)).to(self.device)
                    if pa.ndim == 2:
                        pa = pa.unsqueeze(0)  # (H, D) -> (1, H, D)
                    raw = self.model.predict_action_with_training_rtc_flow(
                        observations=observations,
                        states=states,
                        instructions=[instruction],
                        num_inference_steps=8,
            ...
            raw = raw.reshape(-1, self.Da).float().cpu().numpy()  # (H, Da) NORMALISED
            denorm = self.maxmin.denormalize(raw.copy())          # (H, Da) absolute joint targets
            denorm[..., -5] = 0.75  # torso-height hardcode (matches /act; irrelevant to arm/hand) ### ZERO OUT ###

            out = {"action_norm": raw.astype(np.float32), "action_denorm": denorm.astype(np.float32)}
            overwatch.info(f"[rtc] Return chunk {raw.shape} (rtc={has_prev})")
            return JSONResponse(content=convert_numpy_in_dict(out, numpy_serialize))
    ...
    def run(self, host: str = "0.0.0.0", port: int = 8000) -> None:
        ...
        ### CLAUDE ### Stateless RTC endpoint for world-model closed-loop rollouts.
        self.app.post("/act_rtc")(self.predict_action_rtc)
        ### END CLAUDE ###
...
if __name__ == "__main__":
    ...
    color_print("config:", config, style="green")
    serve(config)